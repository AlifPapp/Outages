...
async def on_ready():
    ...
    botstatus = client.botstatus.find_one({"bot_id": 836198930873057290})
    if botstatus is None:
        status = {"bot_id": 836198930873057290, "status": "online"}
...
async def help(ctx):
    em = discord.Embed(title = "Commands",
                           description = f"`{defaultprefix}prefix`, `{defaultprefix}info`, `{defaultprefix}botinfo`",
                           colour = client.Blue)
...
async def Loop_CheckStatus():
    ...
    if client.member.status == discord.Status.offline:
        if status == "online":
            ...
            client.botstatus.update_one({"bot_id":836198930873057290},{"$set":{"status":"offline"}})
    else:
        if status == "offline":
            await sendstatus("online")
            client.botstatus.update_one({"bot_id":836198930873057290},{"$set":{"status":"online"}})
    return
